...
{
    // Static definition
    std::mutex VulkanBuffer::s_VulkanSyncMutex;
	...
	void VulkanBuffer::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, VkImageAspectFlags aspectFlags, int layerCount, uint32_t mipLevels, uint32_t baseMipLevel) {
		...
		// Delegate the actual barrier recording to the lightweight helper to avoid code duplication
		transitionImageLayoutInCommand(
			commandBuffer,
			image,
			format,
			oldLayout,
	...
	void VulkanBuffer::copyBufferToImageWithRegion(VkBuffer buffer, VkImage image, const VkBufferImageCopy& region) {
		std::lock_guard<std::mutex> lock(s_VulkanSyncMutex);
		VkCommandBuffer commandBuffer = beginSingleTimeCommands();
		vkCmdCopyBufferToImage(commandBuffer, buffer, image, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1, &region);
		endSingleTimeCommands(commandBuffer);
	...
	/*
	 * Record an image layout transition directly into an existing command buffer.
	 * This avoids allocating a one-time command buffer and waiting on the queue,
	 * making it suitable for per-frame usage inside the main rendering loop.
	 */
	void VulkanBuffer::transitionImageLayoutInCommand(
		VkCommandBuffer commandBuffer,
		VkImage image,
		VkFormat format,
		...
		if (commandBuffer == VK_NULL_HANDLE) {
			throw std::runtime_error("Provided command buffer is null in transitionImageLayoutInCommand");
		}
		...
		if (oldLayout == VK_IMAGE_LAYOUT_UNDEFINED && newLayout == VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL) {
			...
			throw std::invalid_argument("unsupported layout transition in transitionImageLayoutInCommand!");
		}
